...
%%%%%%%% Les méta-données
\hypersetup{pdfauthor = {\theauthor}, % Auteur : pdfauthor = {\theauthor}
            pdftitle = {Ingénieur Télécom}, % Titre : pdftitle = {Titre du document}
            pdfsubject = {Propagation de la Covid-19}, % Sujet : pdfsubject = {Sujet du document}
            pdfkeywords = {Covid-19, 5G}, % Mots-clés (Thésaurus ?) : pdfkeywords = {mots-clés}
            % pdfproducer = {pdfLaTeX}, % Outil de conversion pdf : pdfproducer = {pdfLaTeX}
            pdfcreator = {Scrib'UTT} % Créateur/Outil pour créer le pdf : pdfcreator = {Scrib'UTT}
}
% Pour plus de méta-données différentes, allez voir la documentation de hyperref : https://ctan.gutenberg.eu.org/macros/latex/contrib/hyperref/doc/hyperref-doc.pdf#subsection.3.7
